...
Sur demande de l'opérateur (« mets-toi à jour par rapport au gabarit »), lire `MIGRATIONS.md` du
dépôt et appliquer **dans l'ordre** toutes les migrations postérieures à la version ci-dessus.

La migration touche la **structure**, jamais le **contenu**. Elle est appliquée par l'agent qui lit
les notes, jamais par un script (P11). Puis mettre ce fichier à jour et **lister ce qui a été
touché**. Une migration silencieuse est une réécriture.
